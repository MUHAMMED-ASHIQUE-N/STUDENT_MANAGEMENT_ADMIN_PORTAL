...
function Students() {
  ...
  const totalFee =
    (Number(selectedCourse?.fees.courseFee) || 0) +
    (Number(selectedCourse?.fees.admissionFee) || 0);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setAdmissionFeeError(`Admission fee must be ₹${selectedCourse.fees.admissionFee}`);    
    return;
  } else {
    ...
    <div className="p-8">
      <div className="w-1/4   -4">
        <input
          ...
          className="outline outline-gray-400 float-left w-full  px-4 py-1 rounded-md "
        />
      </div>
      <button
        onClick={() => setFormOpen(formOpen ? false : true)}
        className="bg-blue-500 text-white rounded-xl px-4 py-2  float-end "
      >
        {" "}
          ...
          >
            <div className="grid grid-cols-2 gap-4 w-full bg- p-6 space-y-1">
              ...
              {planType === "custom" && selectedCourse && (
                <div>
                  <label className="block mb-2 font-semibold">
                    Choose Custom Installments
                  </label>
                  <select
                    onChange={(e) =>
                      handleCustomCheckpoint(Number(e.target.value))
                    }
                    className="border px-2 py-1 rounded"
                  >
                    <option value="">Select Installments</option>
                    {Array.from(
                      { length: selectedCourse?.duration || 0 },
                      (_, i) => i + 1
                    ).map((cp) => (
                      <option key={cp} value={cp}>
                        ...
                        {cp} month{cp > 1 ? "s" : ""} (₹
                        {Math.round(
                          Number(selectedCourse?.fees.courseFee) / cp
                        )}
                        )
                      </option>
                    ))}
              ...
              {selectedCourse && <p>{selectedCourse.fees.admissionFee} </p>}
              <input
                type="text"
                value={admissionFee}
                ...
                className={`border rounded-md py-2 px-3 ${
                  admissionFeeError ? "border-red-500" : "border-gray-400"
                }`}
              />
              {admissionFeeError && (
                <p className="text-red-500 text-sm mt-1">{admissionFeeError} error occureeeee</p>
              )}
            </div>

            <button
              type="submit"
              className="bg-blue-500 px-18 py-2 rounded-xl text-white "
            >
              {loading
                ? "Saving..."
                : editId
